...
require IA_ROOT . '/store/common/bootstrap.sys.inc.php';
load()->store('common');
load()->store('template');
load()->func('check');
...
$controllers = array();
$handle = opendir(IA_ROOT . '/store/source/');
if(!empty($handle)) {
...
$init = IA_ROOT . "/store/source/{$controller}/__init.php";
if(is_file($init)) {
	require $init;
}

$actions = array();
$handle = opendir(IA_ROOT . '/store/source/' . $controller);
if(!empty($handle)) {
	while($dir = readdir($handle)) {
...
function _forward($c, $a) {
	$file = IA_ROOT . '/store/source/' . $c . '/' . $a . '.ctrl.php';
	return $file;
}
