...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/store/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/store/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$source = IA_ROOT . "/store/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/store/default/{$filename}.tpl.php";
	}
