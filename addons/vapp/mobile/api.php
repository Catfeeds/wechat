...
if($op == "get_lives"){
    ...
    $psize = 21;
    $pindex = ($page-1)*$psize;
    $lives = pdo_fetchall("SELECT a.id,a.url,a.thumb,a.look_num,a.updatetime,a.status,b.mobile,b.realname,b.nickname FROM ".tablename('vapp_live')." a LEFT JOIN ".tablename('vapp_member')." b ON a.uid=b.uid ORDER BY a.order_by DESC,a.status DESC LIMIT {$pindex},{$psize}");
    if(check_data($lives)){
        foreach ($lives as $k => &$v){
            $v['thumb'] = tomedia($v['thumb']);
            $v['show_name'] = (!!$v['nickname']?$v['nickname']:(!!$v['realname']?$v['realname']:$v['mobile']));
            $v['updatetime'] = friend_date($v['updatetime']);
        }
...
>>>>>>> 1d5c09804738f93524f6cf59c114df730f1f485a
message('访问出错','','error');