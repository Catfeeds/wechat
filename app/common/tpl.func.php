...
function _tpl_form_field_date($name, $values = '', $withtime = false,$placeholder = '请选择日期') {
	$html = '';
	$html .= '<input class="mui-calendar-picker" type="text" placeholder="'.$placeholder.'" readonly value="' . $values . '" name="' . $name . '" />';
		...
		$html .= '
			<script type="text/javascript">
				$(document).on("tap", ".mui-calendar-picker", function(){
					var $this = $(this);
					util.datepicker({type: "date", beginYear: 1960, endYear: '.date('Y').'}, function(rs){
						$this.val(rs.value)
						.next().val(rs.value)
					});
				});
