...
if($_W['isajax']) {
	...
	//获取管理员信息
	$user_info = pdo_get('sj_news_ad_account',array('username'=>$username));
	if(empty($user_info)){
	...
	$cookie = array();
	$cookie['ad_id'] = $user_info['id'];
	$cookie['ad_hash'] = md5($user_info['password'] . $user_info['salt']);
	$session = base64_encode(json_encode($cookie));
	isetcookie('__ad_session', $session, !empty($_GPC['rember']) ? 7 * 86400 : 0, true);
