...
require IA_ROOT . '/ad/common/bootstrap.sys.inc.php';
load()->ad('common');
load()->ad('template');
...
$controllers = array();
$handle = opendir(IA_ROOT . '/ad/source/');
if(!empty($handle)) {
...
$init = IA_ROOT . "/ad/source/{$controller}/__init.php";
if(is_file($init)) {
	require $init;
}

$actions = array();
$handle = opendir(IA_ROOT . '/ad/source/' . $controller);
if(!empty($handle)) {
	while($dir = readdir($handle)) {
...
function _forward($c, $a) {
	$file = IA_ROOT . '/ad/source/' . $c . '/' . $a . '.ctrl.php';
	return $file;
}
