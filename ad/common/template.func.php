...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/ad/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/ad/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$source = IA_ROOT . "/ad/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/ad/default/{$filename}.tpl.php";
	}
