...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/account/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/account/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$source = IA_ROOT . "/account/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/account/default/{$filename}.tpl.php";
	}
