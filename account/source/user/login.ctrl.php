...
if($_W['isajax']) {
	...
	//获取会计
	$user_info = pdo_get('account_user',array('username'=>$username));
	if(empty($user_info)){
	...
	$cookie = array();
	$cookie['account_id'] = $user_info['id'];
	$cookie['account_hash'] = md5($user_info['password'] . $user_info['salt']);
	$session = base64_encode(json_encode($cookie));
	isetcookie('__account_session', $session, !empty($_GPC['rember']) ? 7 * 86400 : 0, true);
