...
require IA_ROOT . '/vapp/common/bootstrap.sys.inc.php';
load()->vapp('common');
load()->vapp('template');
load()->func('check');
...
$controllers = array();
$handle = opendir(IA_ROOT . '/vapp/source/');
if(!empty($handle)) {
...
$init = IA_ROOT . "/vapp/source/{$controller}/__init.php";
if(is_file($init)) {
	require $init;
}

$actions = array();
$handle = opendir(IA_ROOT . '/vapp/source/' . $controller);
if(!empty($handle)) {
	while($dir = readdir($handle)) {
...
function _forward($c, $a) {
	$file = IA_ROOT . '/vapp/source/' . $c . '/' . $a . '.ctrl.php';
	return $file;
}
