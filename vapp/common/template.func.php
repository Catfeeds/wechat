...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/vapp/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/vapp/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$source = IA_ROOT . "/vapp/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/vapp/default/{$filename}.tpl.php";
	}
