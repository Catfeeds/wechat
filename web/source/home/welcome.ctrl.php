...
$dos = array('platform', 'site', 'mc', 'setting', 'ext', 'solution', 'replystatistics','poster');
$do = in_array($do, $dos) ? $do : $do;
$title = array('platform'=>'公众平台','site'=>'微站功能','mc'=>'会员及会员营销','setting'=>'功能选项','ext'=>'扩展功能','solution'=>'行业功能','poster'=>'海报设置');
...
if($do == 'ext') {
	...
	foreach ($installedmodulelist as $k => &$value) {
		$value['official'] = empty($value['issystem']) && (strexists($value['author'], 'WeEngine Team') || strexists($value['author'], '慕马科技'));
	}
	$m = $_GPC['m'];
	if(empty($m)) {
