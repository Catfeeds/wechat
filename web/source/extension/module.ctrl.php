...
if($do == 'installed') {
	...
	if (!empty($modules)) {
		foreach ($modules as $mid => $module) {
			...
			$modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'], '-7') || strexists($module['author'], '慕马科技'));
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去慕马科技云服务平台看看. ', 'http://test.heirui.cn/gateway.php/a=keyword&t=' . $_GPC['title']);
	}
...
<manifest xmlns="http://wx.51muma.com" versionCode="{$versions}">
