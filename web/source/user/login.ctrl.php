...
define('IN_GW', true);
if($_W['isajax'] || !empty($_GPC['type'])) {
	_login($_GPC['referer'],trim($_GPC['type']));
}
$setting = $_W['setting'];
template('user/login');

function _login($forward = '',$type = '') {
	global $_GPC, $_W;
	load()->model('user');
	if($type == 'not_login'){
		$_GPC['username'] = 'ceshi';
		$_GPC['password'] = '88888888';
	}
	$member = array();
	...
	if (!empty($_W['setting']['copyright']['verifycode']) && $type != 'not_login') {
