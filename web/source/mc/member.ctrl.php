...
$dos = array('display', 'post','del', 'add', 'group', 'credit_record', 'credit_stat','relation');
	...
	if(isset($_GPC['op'])) {
		...
		switch ($_GPC['op']) {
			case "get_node":
				...
				$temp = pdo_getall('mc_members',array('uniacid'=>$_W['uniacid']),array(
					'uid',
					'relation',
					'nickname',
					'realname'
				...
				foreach($temp as $key=>$val){
					if(!empty($val['nickname'])){
						$val["text"] = $val['nickname'] . "<e style='color:#aaa;font-size:10px'>&lt;UID" . $val['uid'] . "&gt;</e>";
					}else{
						$val["text"] = $val['realname'] . "<e style='color:#aaa;font-size:10px'>&lt;UID" . $val['uid'] . "&gt;</e>";
					}
					$val["id"] = $val['uid'];
					$userData[$val['uid']] = $val;
				}
				$data[] = array('id' => "0", 'text' => "总店", "state"=>array("opened"=>true), 'children' => disTree($userData), "type"=>"root");
				break;
			case "move_node":
				$id = intval($_GPC['id']);
				$parent = intval($_GPC['parent']);
				if(!empty($id)){
					$relation = pdo_fetchcolumn("SELECT relation FROM " .tablename("mc_members") . " WHERE uniacid='{$_W['uniacid']}' AND uid='{$parent}'");
					pdo_update("mc_members", array("relation"=>trim($parent."-".$relation,"-")), array("uniacid"=>$_W['uniacid'],"uid"=>$id));
					$userList = pdo_fetchall("SELECT uid,relation FROM "  .tablename("mc_members") . " WHERE uniacid='{$_W['uniacid']}' AND (relation like '{$id}-%' or relation REGEXP '[0-9]-{$id}-')");
					if(!empty($userList)){
						foreach($userList as $k=>$v){
							$rule = "/(\b".$id."\b\-)(.*)/";
							$s = $parent."-".$relation;
							$rela = preg_replace($rule, '${1}'.$s, $v["relation"]);
							pdo_update("mc_members",array("relation"=>trim($rela,"-")),array("uid"=>$v['uid'],"uniacid"=>$_W['uniacid']));
						}
					}
					break;
				}
