...
class ExcelTool{
    ...
    /* 导出 */
    function export($list, $params = array(),$author = "PHPExcel"){
        if (PHP_SAPI == 'cli') {
            die('This example should only be run from a Web Browser');
        }
        require_once IA_ROOT . '/framework/library/phpexcel/PHPExcel.php';
        $excel = new PHPExcel();
        $excel->getProperties()->setCreator($author)->setLastModifiedBy($author)->setTitle("Office 2007 XLSX Test Document")->setSubject("Office 2007 XLSX Test Document")->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")->setKeywords("office 2007 openxml php")->setCategory("report file");
    ...
    public function example(){
        ...
        $this->export(
            $list,//导出的数据数组
            array(
                "title" => "订单数据-" . date("Y-m-d-H-i", time()) , //标题
                "columns" => $columns //数据列
            )
        );
    }
