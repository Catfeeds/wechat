...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://wx.51muma.com/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
	return true;
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://wx.51muma.com/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://wx.51muma.com/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止，请联系慕马科技客服！');
	}
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://wx.51muma.com/gateway.php', $pars);
...
function cloud_cron_update($cron) {
	...
	$pars['cron'] = base64_encode(iserializer($cron));
	$result = cloud_request('http://wx.51muma.com/gateway.php', $pars);
	//http://wx.51muma.com/gateway.php
	return _cloud_cron_parse($result);
}
...
function cloud_auth_ip(){
	...
	$pars['cron_id'] = $cron_id;
	$pars['status'] = $status;
	$result = cloud_request('http://wx.51muma.com/gateway.php?a=ipcl', $pars);
	...
	$auth_url = 'http://wx.51muma.com/gateway.php?a=ipcl&__auth=' . $query;
