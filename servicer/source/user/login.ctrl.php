...
if($_W['isajax']) {
	...
	//获取客服信息
	$user_info = pdo_get('servicer_user',array('username'=>$username));
	if(empty($user_info)){
	...
	$cookie = array();
	$cookie['servicer_id'] = $user_info['id'];
	$cookie['servicer_hash'] = md5($user_info['password'] . $user_info['salt']);
	$session = base64_encode(json_encode($cookie));
	isetcookie('__servicer_session', $session, !empty($_GPC['rember']) ? 7 * 86400 : 0, true);
