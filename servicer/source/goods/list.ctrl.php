...
if($do == 'display'){
    ...
    $pager = pagination(ServicerModel::getOldGoodsCount($goods_name,$store_name,$is_check,$province,$city,$district,false),floor(trim($_GPC['page'])),$psize);
}elseif($do == 'post'){
    ...
    $status = ServicerModel::checkOldGoods($id,floor(trim($_GPC['status'])) == CHECK_PASS?CHECK_PASS:CHECK_NOT_PASS);
    if(!$status){
        message('操作失败','','error');
    }
