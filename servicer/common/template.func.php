...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/servicer/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/servicer/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$source = IA_ROOT . "/servicer/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/servicer/default/{$filename}.tpl.php";
	}
