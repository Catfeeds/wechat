...
if ($do == 'delete') {
	$id = intval($_GPC['id']);
	$media = pdo_get('core_attachment', array('uniacid' => $_W['uniacid'],'agent_id'=>$_W['agent_id'], 'id' => $id));
	if(empty($media)) {
		exit('文件不存在或已经删除');
	}
	load()->func('file');
	...
	pdo_delete('core_attachment', array('uniacid' => $uniacid,'agent_id'=>$_W['agent_id'], 'id' => $id));
	exit('success');
}
...
if ($do == 'local') {
	...
	$condition .= " AND agent_id='{$_W['agent_id']}'";
