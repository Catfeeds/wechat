...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/agent/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/agent/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$source = IA_ROOT . "/agent/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/agent/default/{$filename}.tpl.php";
	}
