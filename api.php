...
class WeEngine
{
            ...
            if ($message['event'] == 'unsubscribe') {
                ...
                if (empty($member)) {
                    if (!isset($setting['passport']) || empty($setting['passport']['focusreg'])) {
                        ...
                        $this->_updateRelationTree($message,$rec['uid']);
                        $this->_focusXJCM($rec['uid']);
                    }
                }
                if (!empty($rec)) {
     ...
     * 更改会员的关系树
     */
    private function _updateRelationTree($message,$uid){
       global $_W;
        ...
        if(!empty($message) && is_array($message)){
            if(isset($message['scene'])){
                if(strpos($message['scene'],'friend') === 0){
                    list($type,$uniacid,$parent_uid) = explode(SPLIT_SCENE_STR,$message['scene']);
                    if($parent_uid != $uid && !empty($parent_uid)){ //不是本人
                        ...
                        pdo_update('mc_members',array('relation'=>$parent_uid.SPLIT_RELATION.$parent_relation),array('uniacid'=>$_W['uniacid'],'uid'=>$uid));
                    }
                }
            }
        }
    ...
    //新晋传媒奖励积分
    private function _focusXJCM($uid){
        global  $_W;
        load()->model('mc');
        load()->func('check');
        load()->func('notice');
        ...
        if(check_data($config)){
            if(!empty($config['setting'])){
                ...
                if(check_data($setting)){
                    //是否已经奖励积分
                    $already = pdo_fetchcolumn("SELECT * FROM ".tablename('sj_news_rebate')." WHERE uniacid='{$_W['uniacid']}' AND uid='{$uid}' AND `type`=0");
                    //首次关注送积分
                    if($setting['focus'] > 0 && !check_data($already)){
                        //增加积分并记录
                        mc_credit_update($_W['member']['uid'],'credit1',$setting['share'],array($_W['member']['uid'],"首次关注，赠送{$setting['share']}积分。"));
                        pdo_insert('sj_news_rebate',array(
                            'uniacid' => $_W['uniacid'],
                            'uid' => $uid,
                            'credit1' => $setting['focus'],
                            'type' => 0,
                            'createtime' => TIMESTAMP
                        ));
                        notice_send_simple_text($uid,"首次关注，恭喜您获得{$setting['share']}积分奖励。【新晋传媒】");
                    }
                }
            }
        }
